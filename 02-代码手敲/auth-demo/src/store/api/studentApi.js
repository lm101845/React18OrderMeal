...
const studentApi = createApi({
    ...
    baseQuery: fetchBaseQuery({
        ...
        prepareHeaders: (headers,{getState})=>{
            //获取用户token(localStorage中的token只是作为一个备份)
            //真正有效的token应该是从state中,redux里面的
            //而useSelector只能在组件中或自定义钩子中才可以使用，这里是一个函数，用不了
            //但这里它还有一个参数，里面有
            //用来统一设置请求头
            ...
            getStudents: build.query({
                query() {
                    ...
                    return {
                        url:'students',
                        headers:{
                            // Authorization:"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MiwiaWF0IjoxNjY0MjYxNjU0LCJleHAiOjE2NjY4NTM2NTR9.t7m_BTWZaPZq9HswITFpyVrsEAxdXw-tRic-S3zd770"
                        }
                    }
                },
